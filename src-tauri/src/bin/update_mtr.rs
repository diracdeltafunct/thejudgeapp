/// update_mtr — download and import the Magic Tournament Rules into judge.db
///
/// Usage:
///   cargo run --bin update_mtr                          # uses default DB path + latest MTR URL
///   cargo run --bin update_mtr -- --db path/to/judge.db
///   cargo run --bin update_mtr -- --file path/to/mtr.pdf
///   cargo run --bin update_mtr -- --url https://... --db path/to/judge.db
...
fn main() {
    ...
    // Get the MTR PDF bytes
    let pdf_bytes = if let Some(path) = file_path {
        println!("Reading from file: {}", path.display());
        std::fs::read(&path).expect("Could not read MTR file")
    } else {
        let target_url = url.as_deref().unwrap_or(MTR_URL);
        println!("Downloading MTR from: {}", target_url);
...
fn download_bytes(url: &str) -> Vec<u8> {
    let client = reqwest::blocking::Client::builder()
        .user_agent("thejudgeapp/0.1 update_mtr")
        .build()
...
fn import(
    conn: &mut Connection,
    parsed: &thejudgeapp_lib::parser::mtr_parser::ParsedMTR,
) -> i64 {
    let tx = conn.transaction().expect("Could not start transaction");

    // Remove any existing MTR document
    tx.execute(
        "DELETE FROM rules WHERE doc_id IN (SELECT id FROM documents WHERE doc_type='mtr')",
        [],
    )
    .unwrap();
    tx.execute(
        "DELETE FROM documents WHERE doc_type='mtr'",
        [],
    )
    .unwrap();
    ...
    tx.execute(
        "INSERT INTO documents (doc_type, version) VALUES ('mtr', ?1)",
